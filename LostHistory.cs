...
{
    public partial class LostHistory : Form
    {
        public LostHistory()
        {
            InitializeComponent();
        ...
        private void LostHistory_Load(object sender, EventArgs e)
        {
            //First datagrideview
            ...
            cmd.CommandText = "SELECT * FROM LostBooks";
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            using (SqlConnection con = new SqlConnection(@"Data Source=HIRUNI;Initial Catalog=LibMSysFirstTry;Integrated Security=True;Encrypt=False"))
            {
                ...
                try
                {
                    ...
                    {
                        query = "SELECT * FROM LostBooks WHERE BookId = @BookId";
                        cmd.Parameters.AddWithValue("@BookId", BookId);
                    }
                    else // Searching by MembersId
                    {
                        query = "SELECT * FROM LostBooks WHERE LastHoldersId = @MembersId";
